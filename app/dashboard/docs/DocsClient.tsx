...
const SECTIONS = [
  ...
  { icon: "🎯", name: "Парсер и прогрев", href: "/dashboard/user-parser", desc: "Парсинг чатов и пользователей + безопасный прогрев аккаунтов." },
];
        ...
        <div className="docs-wrap">
          ...
          <div className="docs-main">
            ...
            <section id="parser" className="docs-section">
              ...
              <p className="docs-p">
                «Телефон» — это номер вашего <b>Telegram-аккаунта</b>, через который идёт
                парсинг (тот же, под которым вы получили <code>api_id</code> и <code>api_hash</code>
                на <b>my.telegram.org</b> → API development tools). Это не токен бота, а живой
                аккаунт. Настоятельно рекомендуем <b>отдельный аккаунт</b>, а не основной: при
                активном парсинге Telegram может временно ограничить номер. Запасной аккаунт
                регистрируется на обычную или виртуальную SIM.
              </p>

              <div className="docs-sub">Три режима парсера</div>
              <ol className="docs-oli">
                <li><b>Чаты по ключевым словам</b> — найти целевые каналы/группы по словам ниши (с AI-подсказками окончаний), с диапазоном участников. Экспорт списка чатов.</li>
                <li><b>Пользователи из чатов</b> — собрать участников из чатов с <b>открытым</b> списком. Фильтры: боты, удалённые, только с юзернеймом / фото / Premium.</li>
                <li><b>По сообщениям (скрытые)</b> — собрать авторов сообщений даже когда список участников <b>скрыт</b>. Настройки: AI-защита (Консервативный/Сбалансированный/Агрессивный), лимит сообщений, период (дней), ключевые слова в сообщениях, учитывать ответы/пересланные.</li>
              </ol>
              <div className="docs-sub">Порядок работы</div>
              <ol className="docs-oli">
                <li>Войдите по аккаунту: api_id + api_hash с my.telegram.org, телефон → код (и пароль 2FA, если включён).</li>
                <li>Выберите режим, укажите чаты/ключевые слова и настройте фильтры.</li>
                <li>«🚀 Запустить парсинг» → список результатов. Скопируйте ссылки или выгрузите в CSV / JSON (для чатов — ещё TXT).</li>
              </ol>
              <div className="docs-callout">
                Вход даёт полный доступ к аккаунту (сессия хранится в браузере). Соблюдайте
                правила Telegram и законы о персональных данных — парсите ответственно, не
            ...
            <section id="warming" className="docs-section">
              ...
              <ol className="docs-oli">
                ...
                <li><b>Интенсивность</b>: «Осторожный» для новых (0–7 дней), «Нормальный» (7–30), «Агрессивный» для старых (30+). Включите автоадаптацию по стадии.</li>
                <li><b>Лимиты безопасности</b>: мягкий старт — до 5 действий/час, ~15/день, короткая сессия 30–60 мин; прогрессивное увеличение (день 1: 30% → день 7: 100%).</li>
                <li><b>Действия</b>: на старте оставьте безопасные (чтение каналов, просмотр профилей, симуляция печати, голосования, архивирование). Реакции, сторис, вступления в группы, диалоги — подключайте позже.</li>
                <li><b>Свои каналы/группы</b> подключайте не сразу, а когда аккаунт пройдёт начальный этап.</li>
                <li><b>Запустите</b> — следите за прогрессом и логом. Если аккаунт замораживается — снизьте число действий, укоротите сессию, уберите спорные действия и дайте больше отлёжки.</li>
              </ol>
              <div className="docs-callout">
                Логика простая: первые дни — минимум, потом плавное усиление. Так прогрев
                выглядит естественно и даёт лучшую выживаемость. Боевой прогрев идёт на
            ...
            <section id="promotion" className="docs-section">
              <div className="section-title">Продвижение</div>
              <p className="muted" style={{ marginTop: 0, maxWidth: 660 }}>
                Полный цикл продвижения в Telegram: найти целевые чаты → собрать аудиторию →
                запустить рассылку и бота. Всё делается в разделе <b>«Парсер Telegram»</b> и
                смежных разделах.
              </p>

              <div className="docs-sub">Шаг 1. Найти целевые чаты по ключевым словам</div>
              <p className="docs-p">
                «Парсер Telegram» → вкладка <b>«Чаты по ключевым словам»</b>. Добавьте
                ключевые слова ниши (например «крипто», «трейдинг»), при желании — окончания
                («новости», «2026»): сервис соберёт комбинации «крипто новости», «крипто 2026»
                и т.д. Задайте диапазон участников (напр. 4 000–100 000) и лимит — и запустите.
                Результат — список каналов и групп с числом участников; выгрузите ссылки в
                TXT / CSV / JSON.
              </p>
              <ol className="docs-oli">
                <li>Чем выше число участников и активность чата — тем качественнее он для рассылки.</li>
                <li>Начинайте с диапазона 4 000–100 000 подписчиков и тестируйте разные ключевые слова.</li>
                <li>Экспорт в TXT даёт чистый список ссылок; JSON/CSV — с названием, username и участниками.</li>
              </ol>

              <div className="docs-sub">Шаг 2. Собрать аудиторию из чатов</div>
              <p className="docs-p">
                Вкладка <b>«Пользователи из чатов»</b>: вставьте ссылки на найденные чаты,
                настройте фильтры (пропустить ботов и удалённых; только с юзернеймом / фото /
                Premium) и запустите — получите список пользователей с экспортом. Работает
                для чатов с открытым списком участников.
              </p>

              <div className="docs-sub">Шаг 3. Запустить рассылку и бота</div>
              <p className="docs-p">
                Соберите бота в <b>«Сценарии»</b> (можно через «✨ Собрать ИИ»), подключите
                каналы в <b>«Каналы»</b> и запустите <b>«Рассылки»</b> по собранной базе.
                Ответы и заявки будут приходить в бота по вашему сценарию.
              </p>

              <div className="docs-callout">
                ⚠ Продвигайте ответственно: соблюдайте правила Telegram и законы о персональных
                данных и рекламе. Не используйте базы для спама — за это блокируют аккаунты.
                Ставьте задержки между действиями, чтобы аккаунт-парсер не попал в бан.
              </div>
              <div className="row" style={{ gap: 10 }}>
                <Link href="/dashboard/user-parser" className="btn btn-ai" style={{ display: "inline-block" }}>Открыть «Парсер Telegram»</Link>
                <Link href="/dashboard/mailings" className="btn btn-primary" style={{ display: "inline-block" }}>К рассылкам</Link>
              </div>
            </section>
